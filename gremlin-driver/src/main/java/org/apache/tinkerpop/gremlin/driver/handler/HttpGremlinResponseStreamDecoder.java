...
public class HttpGremlinResponseStreamDecoder extends MessageToMessageDecoder<DefaultHttpObject> {
    ...
    private static final AttributeKey<Double> BYTES_READ = AttributeKey.valueOf("bytesRead");

    private final MessageSerializer<?> serializer;
    private final double maxResponseContentLength;
    private final ObjectMapper mapper = new ObjectMapper();

    public HttpGremlinResponseStreamDecoder(final MessageSerializer<?> serializer, final double maxResponseContentLength) {
    ...
    protected void decode(ChannelHandlerContext ctx, DefaultHttpObject msg, List<Object> out) throws Exception {
        ...
        if (msg instanceof HttpResponse) {
            ctx.channel().attr(BYTES_READ).set(0.0);
        ...
        if (msg instanceof HttpContent) {
            ...
            Attribute<Double> bytesRead = ctx.channel().attr(BYTES_READ);
